...
class PetriDish:
    def __init__(self, num_cells, chip_proteins, composition=None, t=.035, animate=False):
        """
        Simulation of a whole cell culture. The processes in the Nucleus are run in parallel for a certain number
        of cells. This allows the simulation of ChIP-seq signals
        :param num_cells: Number of simulated cells in the cell culture
        :type num_cells: int
        :param chip_proteins: The proteins that are chipped for. It is expected that this is a list of tuples of the
        form [(protein_name, is_complex_boolean)...] where is_complex_boolean is a flag that signifies whether the
        passed protein name refers to a protein complex.
        :type chip_proteins: List[tuples]
        :param composition: List with integer numbers representing the number of proteins that are simulated per
        nucleus. The order follows the list which is returned by Protein.get_types().
        :type composition: List[int]
        :param t: Temperature representing the amount of movement that is possible for proteins. The positional change
        is a randomly sampled value between [-t, t]
        :type t: float
        :param animate: If true, an animation of the ChIP-seq data is created
        :type animate: bool
        """
        plt.ion()
        ...
        self.chip_proteins = chip_proteins
        self.figure, self.ax = plt.subplots(len(self.chip_proteins), 1)
        self.animate = animate
        self.gif = []

    def chip(self, bins=100, time_step=None):
        """
        Simulate the creation of ChIP-seq data
        :param bins: Number of bins that are used for plotting
        :type bins: int
        :param time_step: The current time step. If none, the title does not have temporal information
        :type time_step: int
        :return: None
        """
        chip_seq = []
        with multiprocessing.Pool(np.maximum(multiprocessing.cpu_count() - 2, 1)) as parallel:
            for pt, is_complex in self.chip_proteins:
                fetched_protein = parallel.starmap(Nucleus.get_associated, zip(
                    self.culture, [pt] * len(self.culture), [is_complex] * len(self.culture)))
                fetched_protein = [f for fetch in fetched_protein for f in fetch]
                chip_seq.append(fetched_protein)

        for i, (cs, pt) in enumerate(zip(chip_seq, self.chip_proteins)):
            if not pt[1]:
                color = Protein.COLORS[pt[0]]
            else:
                color = Protein.COLORS[pt[0].split('_')[0]]
            self.ax[i].hist(cs, bins=bins, range=(0, 1), color=color)
            self.ax[i].set_title('ChIP %s' % pt[0])
        self.figure.suptitle('%s' % (' Time Step ' + str(time_step)) if time_step is not None else '')
    ...
    def simulate(self):
        """
        Simulate molecular processes for all cells in petri dish in parallel
        :return: None
        """
        with multiprocessing.Pool(np.maximum(multiprocessing.cpu_count() - 2, 1)) as parallel:
